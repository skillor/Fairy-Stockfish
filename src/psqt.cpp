...
void init(const Variant* v) {
  ...
  for (PieceType pt : v->pieceTypes)
  {
    ...
    int score_mg = PieceValue[MG][pt];
    int score_eg = PieceValue[EG][pt];

    // Override variant piece value
    if (v->pieceValue[MG][pt] != INT_MAX)
      score_mg = v->pieceValue[MG][pt];
      
    if (v->pieceValue[EG][pt] != INT_MAX)
      score_eg = v->pieceValue[EG][pt];
      

    EvalPieceValue[MG][pc] = EvalPieceValue[MG][~pc] = Value(score_mg);
    EvalPieceValue[EG][pc] = EvalPieceValue[EG][~pc] = Value(score_eg);

    if (EvalPieceValue[MG][pt] > EvalPieceValue[MG][strongestPiece])
        strongestPiece = pt;
  }

  Value maxPromotion = VALUE_ZERO;
  for (PieceType pt : v->promotionPieceTypes)
      maxPromotion = std::max(maxPromotion, EvalPieceValue[EG][pt]);

  for (PieceType pt : v->pieceTypes)
  {
    Piece pc = make_piece(WHITE, pt);
    int score_mg = EvalPieceValue[MG][pt];
    int score_eg = EvalPieceValue[EG][pt];
    ...
    Score captureScore = make_score(mg_value(score), eg_value(score));

    // For drop variants, halve the piece values to compensate for double changes by captures
    if (v->capturesToHand)
        score = score / 2;

    Score evalScore = make_score(mg_value(score), eg_value(score));
    ...
    CapturePieceValue[MG][pc] = CapturePieceValue[MG][~pc] = mg_value(captureScore);
    CapturePieceValue[EG][pc] = CapturePieceValue[EG][~pc] = mg_value(captureScore);

    EvalPieceValue[MG][pc] = EvalPieceValue[MG][~pc] = mg_value(evalScore);
    EvalPieceValue[EG][pc] = EvalPieceValue[EG][~pc] = mg_value(evalScore);
    ...
    for (Square s = SQ_A1; s <= SQ_MAX; ++s)
    {
        ...
        Score psqScore = score + (  pt == PAWN  ? PBonus[std::min(r, RANK_8)][std::min(file_of(s), FILE_H)]
        ...
        if (pt == SOLDIER && r < v->soldierPromotionRank)
            psqScore -= score * (v->soldierPromotionRank - r) / (4 + f);
        // Corners are valuable in reversi
        if (v->enclosingDrop == REVERSI)
        {
            if (f == FILE_A && (r == RANK_1 || r == v->maxRank))
                psqScore += make_score(1000, 1000);
        }
        // In atomic variants pieces are "self-defending" and should therefore be pushed forward
        if (v->blastOnCapture)
            psqScore += make_score(40, 0) * (r - v->maxRank / 2);
        // Safe king squares
        if (r == RANK_1 && f <= FILE_B && ((pt == KING && v->checkCounting) || (pt == COMMONER && v->blastOnCapture)))
            psqScore += make_score(100, 0);

        psq[ pc][s] = make_score(score_mg + v->pieceSquareBonus[MG][pt][s] + (int(mg_value(psqScore)) - score_mg) * int(mg_value(v->psqValue)) / CentiValue,
                                 score_eg + v->pieceSquareBonus[EG][pt][s] + (int(eg_value(psqScore)) - score_eg) * int(eg_value(v->psqValue)) / CentiValue);
